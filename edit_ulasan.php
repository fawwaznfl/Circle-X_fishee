...
<style>
  ...
  a.btn_1.red.approve:hover,
  .btn_1.red.approve:hover {
    background: #f53131;
    color: #fff;
  }
  a.btn_1.red.delete:hover,
  .btn_1.red.delete:hover {
    background: #e83333;
    color: #fff;
  }
...
$id = null; // Inisialisasi ID ke null
$balasan = ''; // Initialize $balasan
...
if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET["id"])) {
    ...
    $sql = "SELECT id, balasan FROM ratee WHERE id = $id";
    ...
    if ($result) {
        ...
        // Check if "balasan" is set in the fetched row
        $balasan = isset($row["balasan"]) ? $row["balasan"] : '';
    } else {
        die("SQL Error: " . mysqli_error($conn));
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validasi input dan mencegah SQL injection
    $id = mysqli_real_escape_string($conn, $id);
    $balasan = mysqli_real_escape_string($conn, $balasan);

    // Query SQL untuk memperbarui kolom "balasan"
    $sql = "UPDATE ratee SET balasan = '$balasan' WHERE id = $id";

     if (mysqli_query($conn, $sql)) {
        echo "Data berhasil diperbarui.";
        //header("Location: list_ulasan_partner.php");
    } else {
...
<body>
    ...
    <form action="edit_ulasan.php" method="post">
        ...
        <label for="balasan">Beri balasan</label> <br>
        <textarea name="balasan" style="width: 80%; height: 100px;" ><?php echo $balasan; ?></textarea> <br>
        <button type="submit" class="btn_1" style="margin-top:20;">Balas</button>
        <a href="list_ulasan_partner.php" class="btn_1 red" style="margin-left:5px;">Batal</a>
    </form>
</body>
</html>